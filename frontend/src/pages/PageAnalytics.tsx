        ...
        {!preview ? (
          <>
            <p style={{ fontSize: 13, color: 'var(--text-mid)', marginBottom: 20 }}>
              Выберите разделы которые войдут в итоговый отчёт:
            </p>
            ...
            {error && (
              <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.3)', borderRadius: 8, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <button className="btn btn-amber" onClick={handleDownload} disabled={generating} style={{ flex: 1, justifyContent: 'center' }}>
                {generating
                  ? <><span style={{ animation: 'spin-slow 1s linear infinite', display: 'inline-block' }}>⟳</span> Генерация...</>
                  : <><Icon name="download" size={14} /> Сформировать PDF</>}
              </button>
            </div>
          </>
        ) : (
          <>
            <div style={{ background: 'var(--bg-deep)', border: '1px solid var(--border)', borderRadius: 8, padding: 24, marginBottom: 20, minHeight: 360, maxHeight: '60vh', overflowY: 'auto' }}>
              <div style={{ maxWidth: 520, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: 20, paddingBottom: 20, borderBottom: '1px solid var(--border)' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-mid)', fontFamily: 'var(--mono)' }}>
                    SemanticAnalyzer v1.0 · {new Date().toLocaleDateString('ru-RU')}
                  </div>
                </div>
                {sections.summary && btResult && (
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-mid)', lineHeight: 1.8 }}>
                      Проанализировано <b style={{ color: 'var(--text-hi)' }}>{btResult.total_docs} документов</b>, выявлено <b style={{ color: 'var(--text-hi)' }}>{btResult.topics.length} тематических кластеров</b>. Когерентность: {btResult.coherence}%. Шум: {btResult.noise_pct}%.
                    </div>
                  </div>
                )}
                {sections.stats && btResult && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--amber)', marginBottom: 6, borderLeft: '3px solid var(--amber)', paddingLeft: 8 }}>2. Статистика</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                      {([
                        [String(btResult.total_docs), 'Документов'],
                        [String(btResult.topics.length), 'Тем'],
                        [`${btResult.coherence}%`, 'Когерентность'],
                ...
                {sections.topics && btResult && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--green)', marginBottom: 6, borderLeft: '3px solid var(--green)', paddingLeft: 8 }}>3. Тематический анализ</div>
                ...
                {sections.anomalies && btResult && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--red)', marginBottom: 6, borderLeft: '3px solid var(--red)', paddingLeft: 8 }}>4. Аномалии и шум</div>
                    <div style={{ fontSize: 11, color: 'var(--text-mid)', lineHeight: 1.8 }}>
                      Нераспознанных документов: <b style={{ color: 'var(--text-hi)' }}>{btResult.noise_count}</b> ({btResult.noise_pct}% от корпуса).
                    </div>
                  </div>
                )}
              </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button className="btn btn-ghost" onClick={() => setPreview(false)} style={{ flex: 1, justifyContent: 'center' }}>
                ← Назад к настройкам
              </button>
...
const PageAnalytics = ({ btResult }: PageAnalyticsProps) => {
  ...
  const dynamicsData = (() => {
    ...
    return Object.entries(byDate)
      ...
      .map(([date, topicCounts]) => ({ date, topicCounts }))
  })()
  ...
  const vosParameters = {
    ...
    min_link_strength_large_network: 0,
    show_isolated_items: true,
  }
    ...
    return (
      <div className="animate-fadeUp" style={{ textAlign: 'center', padding: '80px 0' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-mid)' }}>
          Вернитесь на шаг назад и запустите BERTopic анализ
        </p>
      </div>
    )
  }
        ...
        <div className="animate-fadeIn">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {barData.length > 0 && (
                <div className="card">
                  ...
                  <p style={{ fontSize: 10, color: 'var(--text-lo)', textAlign: 'center', marginTop: 6, fontFamily: 'var(--mono)' }}>
                    количество документов на тему
                  </p>
                </div>
              )}
              ...
              <div className="card">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600 }}>Топик -1</div>
                    <div style={{ fontSize: 10, color: 'var(--text-mid)' }}>Нераспознанные документы</div>
                  </div>
                  <span className={`badge badge-${noisePct > 10 ? 'red' : 'amber'}`}>{noiseCount} doc</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0' }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600 }}>Процент шума</div>
                    <div style={{ fontSize: 10, color: 'var(--text-mid)' }}>От общего корпуса</div>
                  </div>
          ...
          {dynamicsData && dynamicsData.length > 0 && (
            <div className="card" style={{ marginBottom: 20 }}>
              <p className="field-label" style={{ marginBottom: 12 }}>
                Динамика тем · по датам из документов
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {dynamicsData.map(({ date, topicCounts }) => (
                  <div key={date} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text-lo)', minWidth: 90, flexShrink: 0 }}>
                      {date}
                    </span>
              ...
              <p style={{ fontSize: 10, color: 'var(--text-lo)', marginTop: 10, fontFamily: 'var(--mono)' }}>
                // даты извлечены Natasha · наведите на блок для подсказки
              </p>
            </div>
          )}
          ...
          <div className="card" style={{ borderLeft: '3px solid var(--cyan)' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-mid)', lineHeight: 1.8 }}>
              Корпус из <span style={{ color: 'var(--cyan)', fontWeight: 700 }}>{totalDocs} документов</span> разбит
              на <span style={{ color: 'var(--cyan)', fontWeight: 700 }}>{topics.length} тематических кластеров</span>.
              {dominantTopic && <>
                {' '}Доминирующая тема — <b style={{ color: 'var(--text-hi)' }}>«{dominantTopic.label}»</b>
                {' '}({dominantPct}% документов, {dominantTopic.count} ед.).
              </>}
              {' '}Когерентность {coherence}% —
              {coherence >= 70
                ? ' высокая, что свидетельствует о чётком тематическом разграничении.'
                : coherence >= 40
                  ? ' средняя, темы частично перекрываются.'
                  : ' низкая, рекомендуется пересмотреть параметры анализа.'}
              ...
              {dynamicsData && dynamicsData.length > 0 && <>
                {' '}Временной охват: <span style={{ color: 'var(--cyan)', fontWeight: 700 }}>{dynamicsData[0].date}</span> — <span style={{ color: 'var(--cyan)', fontWeight: 700 }}>{dynamicsData[dynamicsData.length - 1].date}</span>.
              </>}
            </p>
          </div>
        </div>
        ...
        <div className="animate-fadeIn">
          {vosData && vosData.network?.items?.length >= 3 ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div className="card">
                  ...
                  {([
                    ['Узлов',     String(vosData.network?.items?.length ?? 0)],
                    ['Связей',    String(vosData.network?.links?.length ?? 0)],
                ...
                <div className="card">
                  ...
                  <p style={{ fontSize: 11, color: 'var(--text-mid)', lineHeight: 1.6 }}>
                    Колёсико мыши — масштабирование. Перетаскивание — перестройка графа.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text-mid)' }}>
              ...
              <p style={{ fontSize: 13 }}>
                VOSviewer требует минимум 3 темы.<br />
                Сейчас: <b style={{ color: 'var(--amber)' }}>{vosData.network.items.length}</b> — загрузите больше документов.
              </p>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text-mid)' }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>🔗</div>
              <p style={{ fontSize: 13 }}>VOSviewer данные не получены — запустите BERTopic анализ</p>
            </div>
          )}
        </div>
      )}
