...
function App() {
  ...
  // ── Тезаурус ──────────────────────────────────────────────────────────────
  // Хранится здесь (в App) чтобы пережить переходы между страницами.
  // Передаётся в PageUpload для отображения/редактирования.
  // Передаётся в PageNatasha → bridge.run_natasha() внутри settings.
  ...
  const pages = [
    ...
    />,
    // PageNatasha получает thesaurus → добавляет в settings при вызове run_natasha
    <PageNatasha
      settings={natSettings}
      setSettings={setNatSettings}
      setNatashaDocs={setNatashaDocs}
      thesaurus={thesaurus}
    />,
    <PageBERTopic
      btSettings={btSettings}
      setBtSettings={setBtSettings}
      setBtResult={setBtResult}
    />,
    <PageAnalytics
      btResult={btResult}
  ...
  return (
    <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1 }}>
      ...
      <div style={{
        ...
        background: 'rgba(7,11,20,.8)', backdropFilter: 'blur(10px)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--cyan)' }}>
            SemanticAnalyzer
          </span>
        </div>
        <div style={{ flex: 1 }} />
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {/* Индикатор тезауруса в топбаре */}
          {thesaurus && (
            <span className="badge badge-amber" title="Тезаурус активен">
              📚 {Object.keys(thesaurus).length} записей
            </span>
          )}
          {btResult && (
            <span className="badge badge-green">{btResult.topics.length} тем</span>
          )}
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '36px 24px 60px' }}>
        ...
        {step === 0 && files.length === 0 && (
          <p style={{
            ...
            color: 'var(--text-lo)', marginTop: 10, fontFamily: 'var(--mono)'
          }}>
        ...
        <div style={{
          marginTop: 60, textAlign: 'center', fontSize: 11,
          color: 'var(--text-lo)', fontFamily: 'var(--mono)',
